...
- **Статус:** open (частично)
- **Связанные решения:** ADR-007 + ADR-008. В G7 Пакет 3 (коммит `c515951`) добавлено `pending_price_resolution` предупреждение. Поля `stock_price_rub` и `listings` — **не добавлены**, остаются в очереди (G12 технический долг или отдельная задача).
