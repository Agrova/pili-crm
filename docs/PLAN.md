...
| v24 | 2026-05-06 | **Принят ADR-018 addendum-unique-products.** Закрывает развилку D ADR-018 + запись 2026-05-05 в `schema-gaps.md`. Принят D2 + β: новая таблица `orders_unique_purchase`, расширение `orders_order_item` / `procurement_purchase_item` / `warehouse_receipt_item` симметричными NULLABLE FK + CHECK; уникальный товар проходит весь поток order → procurement → warehouse, `match_shipment` адаптируется. `warehouse_stock_item` не расширяется. Намерение по distribution shipping + `actual_weight` backfill зафиксировано как направление для G6 (открытый вопрос в `06_open_questions.md`, зависит от ADR-F01). G19 расширяется новой подзадачей G19.7 (адаптация `match_shipment`). G19.3 разблокирован — можно запускать. |
| v25 | 2026-05-06 | **G19.1 закрыта** — `calculate_price` MCP-tool (24-й tool). Read-only, принимает каталожные и inline-уникальные позиции, `rate_override`, `customer_id`. Оборачивает `calculate_retail_price`/`calculate_manufacturer_price` из `app/pricing/service.py`. 10 тестов, mypy strict, ruff clean. `cowork-system-prompt.md` v2.3 (24 tools). |
...
| **G19** | ADR-018: Калькулятор нового заказа | Семь подзадач (после addendum-unique-products): **G19.1** `calculate_price` MCP-tool ✅ 2026-05-06, **G19.2** статус `quoted` (миграция enum + derive-trigger guard + `confirm_quote`), **G19.3** миграция уникальных товаров — `orders_unique_purchase` + расширение `orders_order_item` / `procurement_purchase_item` / `warehouse_receipt_item` симметричными NULLABLE FK + CHECK + `pricing_price_calculation.product_id NULLABLE` (по addendum-unique-products), **G19.4** `message_templates` таблица + базовый шаблон + `get_message_template`, **G19.5** артефакт `order_calculator.html` (первый write-артефакт в проекте), **G19.6** обновление `cowork-system-prompt.md`, **G19.7** (новая) — адаптация `match_shipment` под уникальные товары (UNION-поиск по `orders_unique_purchase.name`, `is_unique` в результате). Зависит от ADR-018 + двух его аддендумов: addendum-exchange-rate (поведение курса в G19.1 / G19.5) и addendum-unique-products (схема, миграция G19.3, расширение G19.7). | Cowork-pf → Claude Code (несколько подзадач) | Sonnet 4.6 + Medium-High | — | 🟠 G19.1 ✅ 2026-05-06. G19.2 / G19.3 / G19.4 — в очереди (G19.2+G19.4 параллельны). |
...
- **Цель:** реализовать ADR-018 + ADR-018 addendum-exchange-rate-update-policy + ADR-018 addendum-unique-products **шестью** подзадачами:
  1. **G19.1 `calculate_price` MCP-tool** ✅ **Закрыто 2026-05-06.** 24-й tool, read-only. Принимает каталожные (product_id) и inline-уникальные позиции, `rate_override` (Decimal-строка), `customer_id`. Оборачивает `calculate_retail_price`/`calculate_manufacturer_price` из `app/pricing/service.py` без дублирования формулы. `rate_source: "db"|"override"` (порог 0.0001). 10 тестов зелёные, mypy strict, ruff clean. `cowork-system-prompt.md` v2.3 (24 tools). Замечание по реализации: цена каталожного товара берётся из последней `catalog_listing_price` (LATERAL join) — уточнить паттерн после пуша, сверить с `app/warehouse/services.py::build_price_input`.
  ...
  3. **G19.3 миграция уникальных товаров** — по addendum-unique-products: `CREATE TABLE orders_unique_purchase` + ALTER на `orders_order_item` / `procurement_purchase_item` / `warehouse_receipt_item` (drop NOT NULL на product_id, add unique_purchase_id NULLABLE FK, CHECK «ровно один из двух», partial index) + ALTER на `pricing_price_calculation` (product_id → NULLABLE). Расширение `crm-mcp/mirror/` новой вкладкой `orders_unique_purchase`. Расширение `create_order` — атомарный INSERT в `orders_unique_purchase` + `orders_order_item` для inline-позиций.
  4. **G19.4 `message_templates`** — таблица + base seed для `code='quote_to_client'` + `get_message_template` MCP-tool.
  5. **G19.5 артефакт `order_calculator.html`** — первый write-артефакт; использует все вышеперечисленные tools + существующие find_customer/create_customer/search_products/get/set_exchange_rate/create_order. **По ADR-018 addendum-exchange-rate:** возраст курса не показывается (только цифра); правка курса в форме локальная, не пишется в БД при правке; при создании заказа артефакт инициирует **два отдельных** Cowork-диалога: (1) подтверждение заказа, (2) только если курс отличается — отдельное подтверждение «обновить курс в БД?» с вызовом `set_exchange_rate`. Без порога — любое отличие. **По ADR-018 addendum-unique-products:** UI поддерживает «+ из каталога» и «+ уникальный»; форма уникального — name/weight_kg/purchase_price_fcy/purchase_currency/source_url; запись в БД через атомарный `create_order`.
  6. **G19.6 обновление промта** — раздел «Калькулятор нового заказа» в `cowork-system-prompt.md`, включая подраздел «Политика курса валют» (краткая формулировка п.1–9 addendum-exchange-rate) и «Уникальные товары в калькуляторе и складском workflow» (что инструменты ищут уникальные через `orders_unique_purchase`, что они проходят весь поток order → procurement → warehouse).
  7. **G19.7 адаптация `match_shipment` под уникальные товары** (новая, по addendum-unique-products, п.9) — расширить SQL UNION-веткой по `orders_unique_purchase.name ILIKE :pat`, добавить `is_unique: bool` в возвращаемые строки, обновить тесты (matched по уникальному, ambiguous между каталожным и уникальным с похожим именем). Без отдельного нового MCP-tool, расширение существующего.
- **Префаза:** `docs/adr/ADR-018-order-calculator-artifact.md` (полный текст), `docs/adr/ADR-018-addendum-exchange-rate-update-policy.md` (обязательное чтение перед G19.1 и G19.5), `docs/adr/ADR-018-addendum-unique-products.md` (обязательное чтение перед G19.1 / G19.3 / G19.5 / G19.7), `docs/adr/ADR-003-addendum-quoted-status.md`, `docs/adr/ADR-004-pricing-profit-policy.md` (формулы, `pricing_rate_id` в breakdown), `docs/adr/ADR-006-derive-status-trigger.md` (где менять guard), `docs/cowork-system-prompt.md` раздел 11 (для соответствия эвристикам).
- **Модель:** Sonnet 4.6 + Medium-High по подзадачам.
- **Зависимости:** ADR-018 + addendum-exchange-rate + addendum-unique-products — все приняты (2026-05-05 / 2026-05-06 / 2026-05-06). G17 (`get/set_exchange_rate`) — **обязателен** для G19.5 (закрыт 2026-05-06, CP16).
- **CP:** CP18
- **Примечание:** G19.1 + G19.2 + G19.4 можно реализовать параллельно (они независимы). G19.3 (уникальные товары) — миграция перед G19.1, потому что схема `CalculatePriceItem` опирается на структуру inline-полей. G19.5 артефакт — после G19.1, G19.2, G19.3, G19.4. G19.7 — после G19.3 (нужна таблица `orders_unique_purchase` для UNION-запроса).
