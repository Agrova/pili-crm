...
- **Цель:** реализовать ADR-018 + ADR-018 addendum-exchange-rate-update-policy + ADR-018 addendum-unique-products **шестью** подзадачами:
  1. **G19.1 `calculate_price` MCP-tool** — единая точка расчёта по правилам ADR-004; принимает items (каталожные ИЛИ inline уникальные) + опциональный `exchange_rate` (если None — берёт из БД). Tool **не пишет** в `pricing_exchange_rate` (см. addendum-exchange-rate, п.2, п.10) и **не пишет** в `orders_unique_purchase` (запись делает `create_order` по addendum-unique-products, п.7-8). Возвращает по позиции (включая `is_unique: bool`) и итог + фактически использованный курс.
