...
- **Суть:** ADR-011 §7 упоминает, что identity-поля `extract` (phone/email/city/name_guess/telegram_username) должны идти «в соответствующие JSONB-поля профиля», но `OrdersCustomerProfile` таких полей не содержит. Реальные целевые поля находятся в базовой таблице `OrdersCustomer` (колонки `phone`, `email`, `telegram_username`). Identity-updates требуют отдельной политики: (а) перезаписывать колонки `OrdersCustomer` при заполненных значениях у клиента; (б) только дополнять при отсутствующих (fill-if-null); (в) игнорировать и полагаться на ручной ввод оператора.
- **Контекст:** обнаружено при реализации ADR-011 Task 2 (`app/analysis/service.py::apply_analysis_to_customer`). Пропущено в Task 2, т.к. требует решения по политике перезаписи. В коде расставлен TODO-комментарий со ссылкой на эту запись.
